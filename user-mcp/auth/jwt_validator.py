...
def extract_identity(claims: dict[str, Any]) -> dict[str, Any]:
    ...
    if scope_claim is None:
        ...
        if isinstance(scp_claim, list):
            ...
            scope_claim = ""
    # RFC 8693 "act" claim, stamped by the keycloak-providers SPI from the
    # Vault-signed actor token sent as the delegation_actor form field during
    # token exchange (see token-exchange/keycloak/keycloak_client.py).
