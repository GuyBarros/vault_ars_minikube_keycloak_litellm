...
with `deploy-k8s/token-exchange.env`, `deploy-k8s/user-mcp.env`,
`deploy-k8s/web-app.env`, and the LiteLLM `GENERIC_CLIENT_SECRET`.
...
attached to the `token-exchange` and `ciba-client` clients) fixes both:
...
- `jwt-keycloak` JWT auth mount — used only to probe the
  CIBA-required-by-policy ACL switch (`ciba/<action>/<user>`, `sys/capabilities-self`).
- `sys/config/oauth-resource-server/keycloak-demo` — Vault 2.1's native OAuth
  Resource Server, which validates the Keycloak OBO/CIBA JWT presented
  directly as `X-Vault-Token` on `database/creds/...` and `transform/encode/...`,
...
| CIBA never resolves | `ciba-channel` pod not reachable at `USER_MCP_CIBA_APPROVE_URL`, or Keycloak's `--spi-ciba-auth-channel-*` flag doesn't point at it |
