...
export async function getAgentAssurance(accessToken: string): Promise<AssurancePayload> {
  ensureConfigured();
  log.debug({ url: agent.assuranceUrl }, 'Fetching agent assurance');
  ...
  try {
    ...
    const rootCause = extractRootCause(err);
    log.error(
      { err: String(err), rootCause: rootCause ? String(rootCause) : undefined, url: agent.assuranceUrl },
      'Agent assurance request failed',
    );
    throw new Error(describeFetchError(err, agent.assuranceUrl));
  }

  if (!res.ok) {
    const body = await extractErrorBody(res);
    log.error({ status: res.status }, 'Agent assurance API returned HTTP error');
    throw new Error(`Agent assurance API error ${res.status}${body ? `: ${body}` : ''}`);
  }
  ...
  const normalized = normalizeAssurancePayload(data);
  if (normalized) return normalized;

  const text = await res.text().catch(() => '');
  const fromText = normalizeAssurancePayload(text);
  if (fromText) return fromText;

  throw new Error('Agent assurance API returned an unexpected response shape.');
}
