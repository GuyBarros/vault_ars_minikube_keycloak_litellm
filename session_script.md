...
* 2. call the script
```
python scripts/demo_forged_jwt_rejected.py
```
* 2.b or directly on vaul
```
JWT_TOKEN=<forged_TOKEN>

scripts/vault-jwt-postgres-user.sh
```
...
```
python3 scripts/opa_gov_latency.py 2>&1 | tail -12
```
...
* apos logar com o usuario e executar a tool que mostra que tudo esta funcionando, revokar o token
```
python3 scripts/revoke_keycloak_token.py --end-session <TOKEN>
```
* apos revocacao, entrar no browser e refazer a chamada
* deslogar e relogar para mostrar que tudo continua trabalhando
 ...
 * com o usuario "user" tente criar um usuario
 * mostar o log que falhou
